...
from quarc.utils.smiles_util import canonicalize_smiles

RDLogger.DisableLog("rdApp.*")

# density
import os
data_path = os.path.join(os.path.dirname(__file__),'../utils/densities_clean_new.tsv')
density_clean = pd.read_csv(data_path, sep="\t")


def get_density(s):
    """Get density from density_clean.tsv, unit is (g/L)"""
    if density_clean is None:
        raise InvalidDensityError("density_clean is None")
...
def canonicalize_smiles_reagent_conv_rules(s):
    ...
    with open(
        "/home/xiaoqis/projects/cond_rec_clean/data/processed/agent_encoder/agent_rules_v1.json", "r"
    ) as f:
