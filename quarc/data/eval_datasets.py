...
from quarc.utils.smiles_util import prep_rxn_smi_input
...
class UnifiedEvaluationDataset:
    ...
    def __init__(
        ...
        self.rxn_class_encoder = ReactionClassEncoder()
    ...
    def __getitem__(self, idx) -> ReactionInput:
        ...
        """
        try:
            targets = self._get_targets(self.data[idx])
        except TypeError as e:
            print(
                f"Error getting targets for reaction{idx}, should discard this reaction: {self.data[idx].document_id}: {e}"
            )
            targets = {
